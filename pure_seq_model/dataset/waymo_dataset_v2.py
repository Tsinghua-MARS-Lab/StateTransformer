...
class WaymoDatasetV2(torch.utils.data.Dataset):
    ...
    # For the input sequence, each 1 second corresponds to one input set.
    ...
    # ego: [1, 5*time_sample_num + 5], feature info:[x, y, heading, cos(heading), sin(heading) ... , length, width, TYPE_VEHICLE, TYPE_PEDESTRAIN, TYPE_CYCLIST]
    # agent: [n, 10*time_sample_num + 5], feature info:[x_ego, y_ego, heading_ego, cos(heading_ego), sin(heading_ego),
    #                                                  x_self, y_self, heading_self, cos(heading_self), sin(heading_self),
    #                                                  ... ,
    #                                                  length, width, TYPE_VEHICLE, TYPE_PEDESTRAIN, TYPE_CYCLIST]
    # ego_anchor: [1, 3*time_sample_num]: feature info: [delta_x, delta_y, delta_heading, ...]
    # agent_anchor: [n, 3*time_sample_num]: feature info: [delta_x_self, delta_y_self, delta_heading_self]
    # agent_valid: [n(64)]

    # label:
    # ego: [1, 3*time_sample_num], feature info:[delta_x, delta_y, delta_heading, ... ,]
    # agent: [n(64), 3*time_sample_num], feature info:[delta_x_self, delta_y_self, delta_heading_self, ... ,]
    # ego_decision: [1], anchor label for ego
    # agent_decision: [n], anchor label for agent
    # agent_to_predict_num: int

    # for the whole scenario, the output data is:
    # Dict[lane: torch.Tensor(batch_size, 8, n, 20, 18), road_line: ..., ..., agent: torch.Tensor(batch_size, 8, n, 10*time_sample_num+5), ...]
    ...
    def generate_from_raw_data(self, index):
        ...
        step_time = self.dataset_config.time_info.time_period_per_data * self.dataset_config.time_info.time_sample_interval
        ...
        anchor_tensor = build_anchor_tensor(self.dataset_config.anchor_param.speed, self.dataset_config.anchor_param.angle, step_time, time_sample_num)
        ...
        ego_anchor_feature_list, ego_decision_label_list = [], []
        agent_anchor_feature_list, agent_decision_label_list = [], []
        for i in range(time_set_num):
            ...
            ego_anchor_feature, ego_decision_label = self.create_anchor_feature_and_decision_based_on_label(ego_label, anchor_tensor)
            ...
            agent_anchor_feature, agent_decision_label = self.create_anchor_feature_and_decision_based_on_label(agent_label, anchor_tensor)
            ...
            ego_anchor_feature_list.append(ego_anchor_feature)
            ego_decision_label_list.append(ego_decision_label)
            agent_anchor_feature_list.append(agent_anchor_feature)
            agent_decision_label_list.append(agent_decision_label)
        ...
        feature_dict = {
            ...
            'ego_anchor_feature': torch.stack(ego_anchor_feature_list).unsqueeze(0),
            'ego_decision_label': torch.stack(ego_decision_label_list).unsqueeze(0),
            'agent_anchor_feature': torch.stack(agent_anchor_feature_list).unsqueeze(0),
            'agent_decision_label': torch.stack(agent_decision_label_list).unsqueeze(0),
        }
    ...
    def get_ego_feature_and_label_in_one_time_set(self, trajs: torch.Tensor, ego_index: int, time_index: int):
        ...
        # build ego label
        ego_label = torch.zeros([1, 3*time_sample_num])
        for i in range(time_sample_num):
            ego_label[0, 0+i*3] = trajs[ego_index, time_index + time_set_interval - i*time_sample_interval, 0]-trajs[ego_index, time_index, 0]  # x
            ego_label[0, 1+i*3] = trajs[ego_index, time_index + time_set_interval - i*time_sample_interval, 1]-trajs[ego_index, time_index, 1] # y
            ego_label[0, 2+i*3] = angle_to_range(
        ...
        for i in range(time_sample_num):
            ...
            ego_feature[:, 3+i*5] = torch.cos(ego_feature[:, 2+i*5]) # cos(heading)
            ego_feature[:, 4+i*5] = torch.sin(ego_feature[:, 2+i*5]) # sin(heading)

            ego_label[:, 0+i*3:2+i*3] = coordinates_to_local_frame(ego_label[:, 0+i*3:2+i*3], rotation_frame)
    ...
    def get_agent_feature_and_label_in_one_time_set(self, trajs: torch.Tensor, object_type: List, ego_index: int, valid_agent_to_predict: List, time_index: int, agent_index: List):
        ...
        agent_feature = torch.zeros([agent_tensor.size(0), 10*time_sample_num + 5])
        for i in range(time_sample_num):
            agent_feature[:, 0+i*10] = agent_tensor[:, time_index - i*time_sample_interval, 0] # x
            agent_feature[:, 1+i*10] = agent_tensor[:, time_index - i*time_sample_interval, 1] # y
            agent_feature[:, 2+i*10] = agent_tensor[:, time_index - i*time_sample_interval, 6] # heading
        agent_feature[:, 10*time_sample_num+0] = agent_tensor[:, time_index, 3] # length
        agent_feature[:, 10*time_sample_num+1] = agent_tensor[:, time_index, 4] # width

        for i in range(agent_tensor.size(0)):
            if agent_type[i] == 'TYPE_VEHICLE':
                agent_feature[i, 10*time_sample_num+2] = 1
            elif agent_type[i] == 'TYPE_PEDESTRIAN':
                agent_feature[i, 10*time_sample_num+3] = 1
            elif agent_type[i] == 'TYPE_CYCLIST':
                agent_feature[i, 10*time_sample_num+4] = 1
        ...
        agent_label = torch.zeros([agent_tensor.size(0), 3*time_sample_num])
        for i in range(time_sample_num):
            agent_label[:, 0+i*3] = agent_tensor[:, time_index + time_set_interval - i*time_sample_interval, 0]-agent_tensor[:, time_index, 0] # x
            agent_label[:, 1+i*3] = agent_tensor[:, time_index + time_set_interval - i*time_sample_interval, 1]-agent_tensor[:, time_index, 1] # y
            agent_label[:, 2+i*3] = angle_to_range(
        ...
        for i in range(time_sample_num):
            # agent feature
            agent_feature[:, 0+i*10:3+i*10] = global_state_se2_tensor_to_local(agent_feature[:, 0+i*10:3+i*10], ego_frame) # x, y, heading in ego frame
            agent_feature[:, 3+i*10] = torch.cos(agent_feature[:, 2+i*10]) # cos(heading) in ego frame
            agent_feature[:, 4+i*10] = torch.sin(agent_feature[:, 2+i*10]) # sin(heading) in ego frame
            ...
            for j in range(agent_feature.size(0)):
                ...
                agent_feature[j, 5+i*10:8+i*10] = global_state_se2_tensor_to_local(agent_feature[j, 0+i*10:3+i*10].unsqueeze(0), agent_frame).squeeze(0) # x, y, heading in agent frame
            agent_feature[:, 8+i*10] = torch.cos(agent_feature[:, 7+i*10]) # cos(heading) in agent frame
            agent_feature[:, 9+i*10] = torch.sin(agent_feature[:, 7+i*10]) # sin(heading) in agent frame

            # agent_label
            agent_label[:, 0+i*3:2+i*3] = coordinates_to_local_frame(agent_label[:, 0+i*3:2+i*3], rotation_frame)

            for j in range(agent_feature.size(0)):
                agent_rotation_frame = torch.tensor([0, 0, agent_feature[j, 2]])
                agent_label[j, 0+i*3:2+i*3] = coordinates_to_local_frame(agent_label[j, 0+i*3:2+i*3].unsqueeze(0), agent_rotation_frame).squeeze(0) # x,y in agent frame
        ...
        for i in range(batch):
            for j in range(int(agent_to_predict_num[i, 0].item())):
                single_pred_dict = {
                    ...
                    'pred_trajs': input_dict['traj_list'][i, :, j, :, 0:2].cpu().numpy(),
    ...
    def create_anchor_feature_and_decision_based_on_label(self, label, anchor_tensor):
        # label: [n, time_sample_num*3], delta_x, delta_y, delta_heading
        # anchor: [k, time_sample_num*3]

        # reshape input to [n, k, time_sample_num, 3]
        time_sample_num = self.dataset_config.time_info.time_sample_num

        label_reshaped = label.view(label.size(0), time_sample_num, 3).unsqueeze(1).repeat(1, anchor_tensor.size(0), 1, 1)
        anchor_reshaped = anchor_tensor.view(anchor_tensor.size(0), time_sample_num, 3).unsqueeze(0).repeat(label.size(0), 1, 1, 1)

        # find anchor with min diff
        x_weight = self.dataset_config.anchor_loss_weight['x']
        y_weight = self.dataset_config.anchor_loss_weight['y']
        heading_weight = self.dataset_config.anchor_loss_weight['heading']

        anchor_diff = torch.abs(label_reshaped - anchor_reshaped)
        loss = x_weight * torch.sum(anchor_diff[:, :, :, 0], dim=2) + \
               y_weight * torch.sum(anchor_diff[:, :, :, 1], dim=2) + \
               heading_weight * torch.sum(anchor_diff[:, :, :, 2], dim=2)

        _, min_loss_index = torch.min(loss, dim=1)
        
        decision = min_loss_index
        anchor_feature_list = [anchor_tensor[min_loss_index[i], :] for i in range(label.size(0))]
        anchor_feature = torch.stack(anchor_feature_list)

        return anchor_feature, decision