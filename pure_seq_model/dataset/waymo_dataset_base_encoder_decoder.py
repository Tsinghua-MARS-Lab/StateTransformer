...
class WaymoDatasetBaseEncoderDecoder(torch.utils.data.Dataset):
    # all info are expressed in target frame
    # targets num may vary in different scenes.
    
    # map:
    # lane: [target_num, n(256), 20, 19], feature info:[x, y, dir_x, dir_y, next_x, next_y, speed_limit, Freeway, SurfaceStreet, BikeLane,
    #        Unknown, Arrow_Stop, Arrow_Caution, Arrow_Go, Stop, Caution, Go, Flashing_Stop, Flashing_Caution]
    # road_line: [target_num, n(64), 20, 14], feature info:[x, y, dir_x, dir_y, next_x, next_y,
    ...
    # road_edge: [target_num, n(64), 20, 8], feature info:[x, y, dir_x, dir_y, next_x, next_y, RoadEdgeBoundary, RoadEdgeMedian]
    # stop_sign+crosswalk+speed_bump: [target_num, n(64), 20, 9], feature info:[x, y, dir_x, dir_y, next_x, next_y, StopSign, Crosswalk, SpeedBump]
    # mask*4: [target_num, n, 20]

    # agent
    # target: [target_num, 1, 5, 12], feature info:[x, y, heading, cos(heading), sin(heading), v_x, v_y, length, width, TYPE_VEHICLE, TYPE_PEDESTRAIN, TYPE_CYCLIST]
    # agent: [target_num, n(64), 5, 12], feature info:[x, y, heading, cos(heading), sin(heading), v_x, v_y, length, width, TYPE_VEHICLE, TYPE_PEDESTRAIN, TYPE_CYCLIST]
    ...
    def generate_from_raw_data(self, index):
        ...
        feature_dict = {
            ...
            'lane_mask': torch.stack(lane_mask_list),
            'road_line_feature': torch.stack(road_line_feature_list),
            'road_line_mask': torch.stack(road_line_mask_list),
            'road_edge_feature': torch.stack(road_edge_feature_list),
            'road_edge_mask': torch.stack(road_edge_mask_list),
            'map_others_feature': torch.stack(map_others_feature_list),
            'map_others_mask': torch.stack(map_others_mask_list),
    ...
    def get_target_feature_and_label(self, trajs: torch.Tensor, target_index: int, target_type: str, time_index: int):
        ...
        target_feature = torch.zeros([1, feature_time_step, 7+5])
        for i in range(feature_time_step):
            target_feature[0, i, 0] = trajs[target_index, time_index - i*feature_time_interval, 0] # x
            target_feature[0, i, 1] = trajs[target_index, time_index - i*feature_time_interval, 1] # y
            target_feature[0, i, 2] = trajs[target_index, time_index - i*feature_time_interval, 6] # heading

            target_feature[0, i, 5] = trajs[target_index, time_index - i*feature_time_interval, 7] # v_x
            target_feature[0, i, 6] = trajs[target_index, time_index - i*feature_time_interval, 8] # v_y
        target_feature[0, :, 7] = trajs[target_index, time_index, 3] # length
        target_feature[0, :, 8] = trajs[target_index, time_index, 4] # width
        if target_type == 'TYPE_VEHICLE':
            target_feature[0, :, 9] = 1
        elif target_type == 'TYPE_PEDESTRIAN':
            target_feature[0, :, 10] = 1
        ...
        rotation_frame = torch.tensor([0, 0, target_info[6]])
        for i in range(feature_time_step):
            target_feature[:, i, 0:3] = global_state_se2_tensor_to_local(target_feature[:, i, 0:3], target_frame)
            target_feature[:, i, 3] = torch.cos(target_feature[:, i, 2]) # cos(heading)
            target_feature[:, i, 4] = torch.sin(target_feature[:, i, 2]) # sin(heading)
            target_feature[:, i, 5:7] = coordinates_to_local_frame(target_feature[:, i, 5:7], rotation_frame)
    ...
    def get_agent_feature_and_label(self, trajs: torch.Tensor, object_type: List, target_index: int, time_index: int):
        ...
        # filter agent by distance if agent num > max_agent_num
        ...
        agent_feature = torch.zeros([agent_tensor.size(0), feature_time_step, 7 + 5])
        for i in range(feature_time_step):
            agent_feature[:, i, 0] = agent_tensor[:, time_index - i*feature_time_interval, 0] # x
            agent_feature[:, i, 1] = agent_tensor[:, time_index - i*feature_time_interval, 1] # y
            agent_feature[:, i, 2] = agent_tensor[:, time_index - i*feature_time_interval, 6] # heading
            agent_feature[:, i, 5] = agent_tensor[:, time_index - i*feature_time_interval, 7] # v_x
            agent_feature[:, i, 6] = agent_tensor[:, time_index - i*feature_time_interval, 8] # v_y
        agent_feature[:, :, 7] = agent_tensor[:, time_index, 3].unsqueeze(1) # length
        agent_feature[:, :, 8] = agent_tensor[:, time_index, 4].unsqueeze(1) # width

        for i in range(agent_tensor.size(0)):
            if agent_type[i] == 'TYPE_VEHICLE':
                agent_feature[i, :, 9] = 1
            elif agent_type[i] == 'TYPE_PEDESTRIAN':
                agent_feature[i, :, 10] = 1
        ...
        for i in range(feature_time_step):
            # agent feature
            agent_feature[:, i, 0:3] = global_state_se2_tensor_to_local(agent_feature[:, i, 0:3], target_frame) # x, y, heading in ego frame
            agent_feature[:, i, 3] = torch.cos(agent_feature[:, i, 2]) # cos(heading) in ego frame
            agent_feature[:, i, 4] = torch.sin(agent_feature[:, i, 2]) # sin(heading) in ego frame
            agent_feature[:, i, 5:7] = coordinates_to_local_frame(agent_feature[:, i, 5:7], rotation_frame) # vel_x, vel_y in ego frame
