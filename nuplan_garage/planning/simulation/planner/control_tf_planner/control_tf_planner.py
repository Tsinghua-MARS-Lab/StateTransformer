...
class ControlTFPlanner(AbstractPlanner):
    ...
    def __init__(self,
                 ...
                 project_to_lane=True,
                 map_radius: float = 50,  # following default PDM offset Model
                 debug: bool=False,
                 low_filter: bool=False,
                 low_filter_threshold: float=0.1,
                 **kwargs):

        self.use_backup_planner = use_backup_planner
        self.project_to_lane = project_to_lane
        ...
        self.low_filter_threshold = low_filter_threshold
        self._iteration = 0
        self.road_dic = None
        del scenario
    ...
    def compute_planner_trajectory(self, current_input) -> List[AbstractTrajectory]:    
        ...
        idm_reference_trajectory = self.idm_planner.compute_planner_trajectory(current_input)._trajectory \
                                    if self.use_backup_planner else None    
        ...
        with torch.no_grad():
            ...
            output = self.model.generate(
                ...
                idm_reference_global=idm_reference_trajectory,
                project_to_lane=self.project_to_lane,
            )
            pred_traj = output[0, -pred_length:].detach().cpu()  # (80, 2) or (80, 4)
            pred_key_points = output[0, :-pred_length].detach().cpu()
        ...
        # # post-processing
        if self.low_filter:
            print("low control signal filtering...")
            filtered_traj = np.zeros_like(pred_traj)
            last_pose = None
            for idx, each_pose in enumerate(pred_traj):
                if last_pose is None:
                    dx, dy = each_pose[:2]
                    dx = 0 if dx < self.low_filter_threshold else dx
                    dy = 0 if dy < self.low_filter_threshold else dy
        ...
        # Debug: save raster
        if self._iteration == 1 and self.debug:
            input_dic = {
