...
class STRTrajectoryGenerator:
    ...
    def predict_states(self, planner_input: PlannerInput) -> List[List[EgoState]]:
        model_samples = [
    ...
    def predict_states_in_batch(self, planner_inputs: list[PlannerInput], map_names: list[str], samples_in_batch: dict=None) -> List[List[EgoState]]:
        ...
        states = [
            ...
            for trajectory, planner_input in zip(trajectories, planner_inputs)
        ]
    ...
    def _compute_str_trajectory(self, model_samples, map_names):
        ...
        with torch.no_grad():
            ...
            def gpu_list_to_cpu_numpy(gpu_list):
                """将 GPU 上的列表转换为 CPU 上的 NumPy 数组。

                Args:
                    gpu_list: GPU 上的 Python 列表，列表中的元素是 PyTorch 张量。

                Returns:
                    CPU 上的 NumPy 数组。
                """
                # 将列表中的所有张量移动到 CPU
                cpu_tensors = [tensor.cpu() for tensor in gpu_list]
                # 将张量列表转换为 NumPy 数组
                cpu_numpy_array = np.array([tensor.numpy() for tensor in cpu_tensors])
                return cpu_numpy_array

            traj_scores = (
                gpu_list_to_cpu_numpy(prediction_generation["traj_scores"])
            )
        ...
        return relative_traj, traj_scores
