...
class Planner(AbstractPDMClosedPlanner):
    def __init__(self, model_path, device, all_road_dic={}, scenarios=None):
        ...
        self.trajectory_sampling = TrajectorySampling(num_poses=80, interval_length=0.1)
        self.proposal_sampling = TrajectorySampling(num_poses=40, interval_length=0.1)
        self.idm_policies = BatchIDMPolicy(
        ...
        self._observation = PDMObservation(
            self.trajectory_sampling, self.proposal_sampling, map_radius
        )
        
        # proposal/trajectory related classes
        self._generator = PDMGenerator(self.trajectory_sampling, self.proposal_sampling)
        self._simulator = PDMSimulator(self.proposal_sampling)
        self._scorer = PDMScorer(self.proposal_sampling)
        self._emergency_brake = PDMEmergencyBrake(self.trajectory_sampling)
        super().__init__(
            self.trajectory_sampling,
            self.proposal_sampling,
            self.idm_policies,
            lateral_offsets,
            map_radius,
        )
    ...
    def inputs_to_model_sample(self, history, traffic_light_data, map_name, planner_input=None):
        ...
        high_res_raster, low_res_raster, context_action, agent_rect_pts_local = self.compute_raster_input(
            ...
            map_name=map_name,
            corrected_route_ids=corrected_route_ids_)
