...
dds_path = "/lpai/volumes/perception/wangyi/poseidon_tools/output/HLX33B120P01266061704611525139"
record = lireplay.Record(dds_path, ['liodometry', '/Val/VehicleStatus', 'rt/chart/behaviors',
                                    'rt/fusion/odometry/obstacles', 'rt/em/fsd_map'], [],
                         False, False)
odom_list = record.getJsonValues('liodometry', "")['liodometry']
print('info: got odom')
vehicle_list = record.getJsonValues('/Val/VehicleStatus', "")['/Val/VehicleStatus']
print('info: got vehicle list')
behavior_list = record.getJsonValues('rt/chart/behaviors', "")['rt/chart/behaviors']
print('info: got behaviors')
fusion_list = record.getJsonValues('rt/fusion/odometry/obstacles', "")['rt/fusion/odometry/obstacles']
print('info: got fusion list')
map_list = record.getJsonValues('rt/em/fsd_map', "")['rt/em/fsd_map']
print('info: got map list')
# frame alignment
# take fusion topic as the standard

# inspect map

print(map_list[0]['hd_map']['traffic_lights'])
exit()

for each_hd_key in map_list[0]['hd_map'].keys():
    # if each_hd_key == 'road_boundaries':
    if each_hd_key != 'lane_boundaries':
        continue

    if isinstance(map_list[0]['hd_map'][each_hd_key], type({})):
        for i, each_element in enumerate(map_list[0]['hd_map'][each_hd_key]):
            if i > 5:
                break
            print(each_hd_key, each_element, map_list[0]['hd_map'][each_hd_key][each_element])
    else:
        print(each_hd_key, map_list[0]['hd_map'][each_hd_key][:5])
print('any map: ', map_list[0]['hd_map'].keys())
exit()

for frame in fusion_list:
    timestamp = frame['header']['stamp']
    # print('fusion ', frame)
    # take odometry as an example
    for odom in odom_list:
        odom_align_flag = False
        if abs(odom['header']['stamp'] - timestamp * 1000) < 50:
            odom_align_flag = True
            break
        if not odom_align_flag:
            continue
    # if odom_align_flag:
    #     print('odom matched: ', odom)
    # else:
    #     print('odom not found')
    for vehicle in vehicle_list:
        vehicle_align_flag = False
        if abs(vehicle['header']['stamp'] - timestamp * 1000) < 50:
            vehicle_align_flag = True
            break
        if not vehicle_align_flag:
            continue
    # if vehicle_align_flag:
    #     print('vehicle matched:', vehicle)
    # else:
    #     print('vehicle not found')
    for behavior in behavior_list:
        behavior_align_flag = False
        if abs(behavior['header']['stamp'] - timestamp * 1000) < 50:
            behavior_align_flag = True
            break
        if not behavior_align_flag:
            continue
    # if behavior_align_flag:
    #     print('behavior matched: ', behavior)
    # else:
    #     print('behavior not found', behavior_list[0])
    # to inspect one sample of the data
    break
