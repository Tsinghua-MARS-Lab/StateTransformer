...
def main(args):
    ...
    all_file_names = [os.path.join(NUPLAN_DB_FILES, each_path) for each_path in os.listdir(NUPLAN_DB_FILES)[:total_file_number] if
                      each_path[0] != '.']
    ...
    if args.city is not None:
        ...
        sorted_file_names = sorted(all_file_names, key=lambda x: os.stat(x).st_size)
        for i, each_file_name in enumerate(sorted_file_names):
            if all_file_names.index(each_file_name) in file_indices:
                sorted_file_indices.append(all_file_names.index(each_file_name))
    # order by processes
    file_indices = []
