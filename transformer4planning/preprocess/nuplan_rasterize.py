...
def static_coor_rasterize(sample, data_path, raster_shape=(224, 224),
    ...
    if "train" in split and aug_rate > 0 and random.random() < aug_rate:
        ...
        if np.all(np.sqrt(check_distance[:,0]**2+check_distance[:,0]**2)>0.20):
            # plt.scatter(ego_pose_agent_dic[frame_id // frequency_change_rate-20: frame_id // frequency_change_rate + 81, :][:,0],
            #     ego_pose_agent_dic[frame_id // frequency_change_rate-20: frame_id // frequency_change_rate + 80, :][:,1],s=6,color='red',label='gt_point')
            max_aug=0.5
            aug_cur=np.array([0,(random.random() * 2 - 1) * max_aug])
            ...
            max_yaw=0.05
            dyaw = (random.random() * 2 * np.pi - np.pi) * max_yaw
            ...
            decay = np.ones((30, 4)) * np.linspace(1, 0, 30).reshape(-1, 1)
            decay[:, 0] *= dx_f
            decay[:, 1] *= dy_f
            decay[:, 2] *= 0
            decay[:, 3] *= dyaw_f
            ego_pose_agent_dic[frame_id // frequency_change_rate+51: frame_id // frequency_change_rate + 80, :] -= decay[1:]

            # plt.scatter(ego_pose_agent_dic[frame_id // frequency_change_rate,0],
            #     ego_pose_agent_dic[frame_id // frequency_change_rate,1],s=9,color='black',label='point')
            # plt.scatter(ego_pose_agent_dic[frame_id // frequency_change_rate-20: frame_id // frequency_change_rate + 81, :][:,0],
            #     ego_pose_agent_dic[frame_id // frequency_change_rate-20: frame_id // frequency_change_rate + 80, :][:,1],s=3,color='blue',label='aug_point')
            # plt.xlabel("X")
            # plt.ylabel("Y")
            # plt.axis('equal')
            # plt.title("point2curve")
            # plt.legend()
            # plt.savefig("enlarge_curve/"+str(frame_id.numpy())+".png")
            # plt.close()
    # initialize rasters
