...
class TrajectoryDecoder(nn.Module):
    def __init__(self, model_args, config):
        ...
        """
        super().__init__()
        self.loss_fct = model_args.loss_fn
        ...
        # traj_decoder and next_token_decoder are all the same in both decoders
    ...
    def forward(self, 
        ...
        if self.ar_future_interval > 0:
            future_key_points = info_dict["future_key_points"] # gt label
            key_points_logits, future_key_points_hidden_state = self.predict_keypoints(hidden_output, info_dict)
            # nuplan single trajectory planning
            if self.k == 1 and self.model_args.task == "nuplan":
            ...
            # waymo multi trajectories prediction
            elif self.k > 1 and self.model_args.task == "waymo":
                ...
                cls_loss = None
                # classification loss in multi trajectories prediction
