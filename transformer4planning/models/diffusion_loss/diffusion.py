...
class TransformerForDiffusion(nn.Module):
    def __init__(self,
        ...
        if obs_as_cond:
            assert time_as_cond
            T_cond += n_cond_steps+n_prior_steps
        ...
        if obs_as_cond:
            ...
            self.prior_emb = nn.Linear(prior_dim, n_emb)
    ...
    def forward(self,
        ...
        prior: Optional[torch.Tensor]= None, **kwargs):
        """
        x: (B,T,input_dim)
        ...
        if self.encoder_only:
            ...
            if self.obs_as_cond:
                ...
                prior_emb = self.prior_emb(prior)
                # (B,To,n_emb)
                cond_embeddings = torch.cat([cond_embeddings, cond_obs_emb, prior_emb], dim=1)
