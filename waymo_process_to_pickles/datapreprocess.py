...
def get_infos_from_protos(data_path, output_path=None, num_workers=8):
    ...
    # src_files = src_files[0:10]
    
    # Split the source files list.
    
    # if len(src_files) == 1000: src_files = src_files[750:1000]
    # else: assert len(src_files) == 150
    ...
    #     data_infos = list(executor.map(func, src_files))
    # data_infos = [process_waymo_data_with_scenario_proto(file, output_path=output_path) for file in src_files]
